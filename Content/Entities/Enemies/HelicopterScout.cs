...
public class HelicopterScout : Character
{
    ...
    public void SpawnBullets()
    {
        ...
        Game.CurrentLevel.SpawnProjectile(scene, LevelRelativePosition + new Vector2(0, 0).Rotated(Rotation), (vel/2) + new Vector2(20, 0).Rotated(Rotation), Rotation + (Mathf.Pi / 2f), 6);
    ...
    public void ShootAtTarget()
    {
        ...
        {
            if (((target as Character).LevelRelativePosition - LevelRelativePosition).Length() < 1200 && ShootState == 0)
            {
                (GetNode("ReloadTimer") as Timer).Start(0.7f);
                ShootState = 1;
            }
        ...
        if (((Timer)GetNode("ReloadTimer")).TimeLeft < 3f && ShootState == 1)
        {
            ShootState = 2;

        }

        else if (((Timer)GetNode("ReloadTimer")).TimeLeft < 2.8f && ShootState == 2)
        {
            //ShootState = 2;
