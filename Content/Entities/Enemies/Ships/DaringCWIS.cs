...
public class DaringCWIS : ShipComponent
{
    ...
    public override void Behavior(float delta)
    {
        ...
        if (((target as Character).LevelRelativePosition - LevelRelativePosition).Length() < TrackRange && (ShootState > 1f || ((Timer)GetNode("ReloadTimer")).TimeLeft < 4.5f))
        {
    ...
    public void SpawnBullets()
    {
        ...
        Game.CurrentLevel.SpawnProjectile(scene, LevelRelativePosition + new Vector2(15, 0).Rotated(Rotation), vel + new Vector2(20, 0).Rotated(Rotation), Rotation + (Mathf.Pi / 2f), BulletDamage);
        Game.CurrentLevel.SpawnProjectile(scene, LevelRelativePosition + new Vector2(15, 0).Rotated(Rotation), vel + new Vector2(20, 0).Rotated(Rotation+offset), Rotation+offset + (Mathf.Pi / 2f), BulletDamage);
    ...
    public void ShootAtTarget()
    {
        ...
        else if (((Timer)GetNode("ReloadTimer")).TimeLeft < 3 && ShootState == 2)
        {
            ...
            if (((Timer)GetNode("ShootTimer")).TimeLeft < 0.1f)
            {
                ...
                (GetNode("ShootTimer") as Timer).Start(0.16f);
