...
public class MissileHostileLongRangeNaval : Projectile
{
    ...
    public override void Behavior(float delta)
    {
        ...
        else if (State>0)
        {
            ...
            float ProgressiveSpeedValue = Mathf.Clamp(Timer / 3.5f, 0f, 1f);
           ...
           // GD.Print("Speeb" + ProgressiveSpeedValue);
            ...
            if ((relative.Length() < SeekerRange) && Mathf.Abs(Velocity.AngleTo(relative)) < Mathf.Deg2Rad(adjustedSeekerAngle))
            {
                ...
                {
                    Velocity = Utils.VecLerp(Velocity, toTarget, 0.005f + (Mathf.Pow(ProgressiveSpeedValue, 4f) * 0.45f));
                }
            }
        ...
        //GD.Print("Timer: " + Timer + ", State: " + State);
    }
